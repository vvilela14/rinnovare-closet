...
function Row({ icon: Icon, label, to, href, badge, onClick, rightExtra }: RowProps) {
  const inner = (
...
export function UserDrawer() {
  ...
  if (!user) {
    return (
      <Link
        to="/auth"
        className="ml-1 inline-flex items-center gap-2 rounded-full bg-primary px-5 py-2 text-xs uppercase tracking-widest text-primary-foreground transition hover:opacity-90"
      >
        Entrar
      </Link>
    );
  ...
  const name = profile?.full_name || `usuário-${user.id.slice(0, 4)}`;
